...
float set_freq()
{
    gpio_set_level(microstep_pin, 1);
    frequency = frequency + ((frequency_change * (20)) / (1000));
    ledc_set_freq(LEDC_MODE, LEDC_TIMER, 4*abs(frequency));
    return frequency;
}
...
void direction()
{
    ...
    if (theta_e_curr > 0)
    {
        gpio_set_level(direction_pin, 0); // Setting the direction pin high
    }
    else
    {
        gpio_set_level(direction_pin, 1); // Setting the direction pin low
    }
}
...
{
    theta_s_curr = theta_s_curr + ((((0.036) * (frequency)) * PI) / 180);
    return theta_s_curr;
}
...
float calculate_theta_e()
{
    if (encoder_value > 0)
    {
        theta_e_curr = theta_e_curr + (((float)encoder_value - 180) * PI / 180);
        return theta_e_curr;
    }
    else
    {
...
void find_angle()
{
    ...
    while (1)
    {
        ...
        encoder_value = (encoder->get_counter_value(encoder)) / 4;
...
void app_main()
{
    ...
    // Creating two tasks : one for finding the angke from the encoder and one for running the motor
    xTaskCreate(&find_angle, "ENCODER RUNNING", 2048, NULL, 5, NULL);
    vTaskDelay(3000 / portTICK_PERIOD_MS);
    xTaskCreate(&run_motor, "MOTOR RUNNING", 4096, NULL, 1, NULL);
}
